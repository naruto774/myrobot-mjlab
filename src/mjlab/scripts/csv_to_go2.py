...
from mjlab.tasks.tracking.config.go2.env_cfgs import go2_flat_tracking_env_cfg
...
class MotionLoader:
  def __init__(
    ...
    self.calf_joint_pos = calf_joint_pos
    self.current_idx = 0
    self.device = device
  ...
  def _load_motion(self):
    ...
    self.motion_dof_poss_input = motion[:, 7:].clone()
    if self.motion_dof_poss_input.shape[1] != len(self.joint_names):
      raise ValueError(
        f"Motion has {self.motion_dof_poss_input.shape[1]} joint columns, "
        f"but Go2 expects {len(self.joint_names)}."
      )

    if self.calf_joint_pos is not None:
      calf_ids = [
        index
        for index, name in enumerate(self.joint_names)
        if name.endswith("_calf_joint")
      ]
      self.motion_dof_poss_input[:, calf_ids] = self.calf_joint_pos
...
def main(
  ...
  calf_joint_pos: float | None = None,
  device: str = "cuda:0",
  render: bool = False,
  line_range: tuple[int, int] | None = None,
):
  ...
  Args:
    ...
    calf_joint_pos: Optional constant Go2 calf reference in radians. By default,
      preserve the per-frame calf positions from the CSV.
  ...
  scene = Scene(go2_flat_tracking_env_cfg().scene, device=device)
  model = scene.compile()
