"""marsdog velocity environment configurations."""
...
MARSDOG_LOCOMOTION_ACTUATOR_NAMES = (
  r".*(fr|fl|rr|rl).*(hip|thigh|calf).*",
  r".*(fr|fl).*tarsus.*",
)


def marsdog_rough_env_cfg(
  play: bool = False,
  ...
  joint_pos_action.scale = {
    name: scale
    for name, scale in MARSDOG_ACTION_SCALE.items()
...
def marsdog_flat_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
  ...
  cfg = marsdog_rough_env_cfg(play=play)
